...
%%-----------------------------------------------------------------------------
%% Converte o oast em east.
%%   oast -> arvore sintatica do ooe.
%%   east -> arvore sintatica do erlang.
transform_oast_to_east(AST, ErlangModuleName, ClassesInfo) ->
...
get_erl_forms(OOErlangForms) ->
    match_erl_form(OOErlangForms, [], []).

match_erl_form([], FunctionList, OtherForms) ->
    {lists:reverse(FunctionList), lists:reverse(OtherForms)};

match_erl_form([Form | OOErlangForms], FunctionList, OtherForms) ->
    case Form of
        {attribute, _Line, class, _ClassName} ->
            match_erl_form(OOErlangForms, FunctionList, OtherForms);
        ...
        {class_methods, _Line, MthdsList} ->
            ...
            match_erl_form(OOErlangForms, N_FunctionList, OtherForms);
        ...
        _AnyOtherForm ->
            ...
            match_erl_form(OOErlangForms, FunctionList, N_OtherForms)
    end.

%%-----------------------------------------------------------------------------
%% Percorre as clausulas dos metodos/funcoes e converte as expressoes de OOErlang
%% para Erlang
