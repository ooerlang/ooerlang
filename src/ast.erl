...
get_ooe_forms(FileName) ->
    Tokens = get_ooe_tokens(FileName),
    lists:map(
    fun(Ts) ->
    ...
    get_ooe_forms_tokens(Tokens)).

%%-----------------------------------------------------------------------------
%% Extrai a lista de Tokens de um arquivo .cerl
get_ooe_tokens(FileName) ->
...
get_ooe_forms_tokens(TokenStream) ->
    get_ooe_forms_tokens(TokenStream, [], []).


get_ooe_forms_tokens([], [], FormStack) ->
    lists:reverse(FormStack);

get_ooe_forms_tokens([], TokenStack, FormStack) ->
    Form = lists:reverse(TokenStack),
    lists:reverse(
    update_form_stack(Form, FormStack));

get_ooe_forms_tokens([DotToken={dot,_}|TokenStream], TokenStack, FormStack) ->
    Form = lists:reverse([DotToken|TokenStack]),
    get_ooe_forms_tokens(TokenStream, [],
    update_form_stack(Form, FormStack));

get_ooe_forms_tokens([Token|TokenStream], TokenStack, FormStack) ->
    get_ooe_forms_tokens(TokenStream, [Token|TokenStack], FormStack).
