...
compile({beam, OOErlangFileName}) ->
    [ErlangFile] = get_erl_file_list([{OOErlangFileName, OOErlangFileName}]),
...
get_erl_file_list(FileNameList) ->
    ASTList = lists:map(fun ast:get_ooe_forms/1, FileNameList),
...
get_erl_file_list([AST | Rest], ClassesInfo, ErlangFileList) ->
    ...
    case core:transform_oast_to_east(AST, ErlangModuleName, ClassesInfo) of
        {ok, ErlangAST} ->
...
get_version() ->
    io:format("OOErlang - A OOP extension for Erlang Compiler ~n"),
